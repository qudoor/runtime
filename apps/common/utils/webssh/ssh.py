...
from apps.common.utils.webssh.tool import get_key_obj
    ...
    def connect(self, host, user, password=None, ssh_key=None, port=22, timeout=30,
        ...
        try:
            ...
            if ssh_key:
                ...
                # https://blog.csdn.net/amo16617/article/details/101522440
                ssh_client.connect(username=user, hostname=host, port=int(port), pkey=key, timeout=timeout)
            else:
                ssh_client.connect(username=user, password=password, hostname=host, port=int(port), timeout=timeout)
