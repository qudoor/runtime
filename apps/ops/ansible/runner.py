...
from .utils import load_extra_vars

from .callback import (
    AdHocResultCallback, PlaybookResultCallBack, CommandResultCallback
)
from ansible.errors import AnsibleError
from .display import AdHocDisplay, PlayBookDisplay
...
Options = namedtuple('Options', [
    ...
    'verbosity', 'check', 'extra_vars', 'passwords',
...
class PlayBookRunner:
    ...
    def __init__(self, inventory=None, options=None, playbook=None):
        """
    ...
    def get_result_callback(self, execution_id=None):
        return self.__class__.results_callback_class(display=PlayBookDisplay(execution_id))
    ...
    def run(self, execution_id=None, extra_vars_opt=None):
        self.results_callback = self.get_result_callback(execution_id)
        self.variable_manager._extra_vars = load_extra_vars(loader=self.loader, extra_vars_opt=extra_vars_opt)
        try:
...
class AdHocRunner:
    ...
    variable_manager_class = VariableManager
    default_options = get_default_options()
    command_modules_choices = ('shell', 'raw', 'command', 'script', 'win_shell', 'setup')
    ...
    def get_result_callback(self, execution_id=None):
        try:
            ...
            logger.error(f'task_id:{execution_id},RESULT_CALLBACK_ERROR:{e}')
    @staticmethod
    ...
    def run(self, tasks, pattern="all", play_name='Ansible Ad-hoc', gather_facts='no', execution_id=None):
        """
...
class CommandRunner(AdHocRunner):
    ...
    modules_choices = ('shell', 'raw', 'command', 'script', 'win_shell', 'setup')
