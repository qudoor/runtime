...
var checkHostName = (rule, value, callback) => {
  ...
  if (!Global.HostNamePattern.test(value)) {
    return callback(new Error(i18n.t("commons.validate.name_not_compliant")))
  }
  callback()
}
...
const HostNameRule = { validator: checkHostName, required: true, trigger: "blur" }
...
export default {
  HostNameRule, NameRule, RequiredRule, SelectRequiredRule, EmailRule, IpRule, NumberWithZeroRule, NumberRule, ClusterNameRule, CommonNameRule, PasswordRule, LengthRule
}
