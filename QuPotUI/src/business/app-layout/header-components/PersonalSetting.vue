  ...
  <el-dropdown trigger="click" @command="handleCommand">
    ...
    <el-dropdown-menu slot="dropdown">
      <div class="personalDropdown">
        <el-dropdown-item class="iconfont icongenggaimima" command="password">{{ $t("commons.personal.change_password") }}
        </el-dropdown-item>
        <el-dropdown-item class="iconfont iconguanyu" command="about">{{ $t("commons.personal.about") }}
        </el-dropdown-item>
    ...
    <el-dialog :title="$t('commons.personal.change_password')" :visible.sync="dialogVisible" width="30%">
      <el-form ref="form" label-position="left" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item style="width: 100%" :label="$t('commons.personal.confirm_password')" prop="password">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submit('form')" v-preventReClick>{{ $t("commons.button.submit") }}</el-button>
      </span>
    </el-dialog>
    <el-dialog class="ko-dialog" :show-close="false" :visible.sync="aboutDialogVisible" width="40%">
      <div class="aboutBackgroundHidden" style="padding: 20px 25px;">
        <el-image v-if="logoAbout === ''" :src="require('@/assets/QuPot-blue.png')" class="sidebar-logo" />
        <el-image v-else :src="logoAbout" class="sidebar-logo" />
        <p>
          <strong>{{ $t("commons.personal.version") }}: {{ this.version }}</strong>
        </p>
      </div>
      <!-- <div style="padding:15px 20px;box-shadow:0px -10px 8px -5px #F0F1F2;" v-if="false">
...
export default {
  ...
  data() {
    return {
      ...
      confirmPassword: "",
      rules: {
        original: [Rule.RequiredRule],
        password: [Rule.RequiredRule, Rule.PasswordRule],
        confirmPassword: [Rule.RequirFedRule, Rule.PasswordRule, {
  ...
  methods: {
    ...
    exit() {
      ...
      //   location.reload()
      // })
      this.$store.dispatch("user-token/logout")
      location.reload()
    },
    // toGithubStar() {
    //   window.open("https://github.com/KubeOperator/KubeOperator", "_blank")
    // },
    async getRole() {
      const { username } = await store.dispatch("user-token/getCurrentUser")
