<template>
  <layout-content :header="$t('task.title')">
    <complex-table ref="clusterData" local-key="cluster_columns" :row-key="getRowKeys" :selects.sync="clusterSelection"
      @selection-change="selectChange" :data="data" :pagination-config="paginationConfig" @search="search"
      v-loading="loading">
      <template #header>
        <el-button-group>
      ...
      <el-table-column sortable :label="$t('task.quproduct')" prop="quproduct" fix />
      <el-table-column sortable :label="$t('runtime.version')" min-width="75" prop="spec.version" fix />
      <!-- <el-table-column sortable :label="$t('runtime.node_size')" min-width="70" prop="nodeSize" /> -->
        ...
        <template v-slot:default="{ row }">
          ...
          <div v-else>
            <div v-if="row.spec.supportGpu === 'enable' || row.spec.supportGpu === 'disable'">
              <el-link type="info" @click="onOpenGpuDialog(row)">{{ $t("commons.status." + row.spec.supportGpu) }}
              </el-link>
            </div>
      ...
      <!-- <el-table-column width="120px" fix>
        <template v-slot:default="{ row }">
          <el-button style="font-weight: bold;" :disabled="row.status !== 'Running'" type="primary" size="mini"
            @click="getDashboardUrl(row)">Dashboard</el-button>
        </template>
      </el-table-column> -->
      <fu-table-operations :buttons="buttons" :label="$t('commons.table.action')" fix />
    </complex-table>

    <el-dialog @close="searchForPolling()" :title="$t('runtime.condition.condition_detail')" destroy-on-close width="70%"
      :visible.sync="dialogLogVisible">
      <task-logs :key="isRefresh" :operation="operationType" :taskName="taskName" @retry="onRetry"
        @cancle="dialogLogVisible = false" />
    </el-dialog>

    <el-dialog :title="$t('runtime.delete.delete_cluster')" width="30%" :visible.sync="dialogDeleteVisible">
      <div v-if="hasOnlyExternal" style="margin-top: 5px;"><span class="input-help">{{
        $t('commons.confirm_message.delete')
      }}</span></div>
      <el-form v-else label-width="120px">
        <div v-if="isKoExternalShow">
          <el-checkbox v-model="isUninstall">{{ $t('runtime.delete.is_uninstall') }}</el-checkbox>
          <div style="margin-top: 5px; margin-bottom: 20px"><span class="input-help">{{ KoExternalNames }} {{
            $t('runtime.delete.sure_uninstall')
          }}</span></div>
        </div>

        <el-checkbox v-model="isForce">{{ $t('runtime.delete.is_force') }}</el-checkbox>
    ...
    <el-dialog :title="$t('runtime.handle_gpu')" width="30%" :visible.sync="dialogGpuVisible">
      ...
      <div v-if="currentCluster.spec.supportGpu === 'disable'">
        ...
        <li style="margin-top: 15px; margin-left: 20px">{{ $t('runtime.handle_gpu_help') }}</li>
      </div>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-if="currentCluster.spec.supportGpu !== 'enable'" size="small" @click="submitGpuHandle('enable')"
          v-preventReClick>{{ $t("commons.button.enable") }}</el-button>
        <el-button v-if="currentCluster.spec.supportGpu !== 'disable'" size="small" @click="submitGpuHandle('disable')"
          v-preventReClick>{{ $t("commons.button.disable") }}</el-button>
      </div>
    </el-dialog>

    <el-dialog :title="$t('runtime.health_check.health_check')" width="50%" :visible.sync="dialogCheckVisible"
      ...
      <div align="center" style="margin-top: 15px">
        <el-table v-loading="checkLoading" :data="checkData.hooks" v-if="!isRecover" border style="width: 90%">
          <el-table-column prop="name" :label="$t('commons.table.name')">
            <template v-slot:default="{ row }">
              {{ $t('runtime.health_check.' + row.name) }}
            </template>
          </el-table-column>
          <el-table-column prop="level" :label="$t('commons.table.status')">
            <template v-slot:default="{ row }">
              {{ $t('runtime.health_check.' + row.level) }}
            </template>
          </el-table-column>
          <el-table-column prop="msg" :label="$t('runtime.health_check.message')" />
        </el-table>
      </div>
      <div align="center" style="margin-top: 15px">
        <el-table v-loading="checkLoading" :data="recoverItems" v-if="isRecover" border style="width: 90%">
          ...
          <el-table-column prop="method" :label="$t('runtime.health_check.method')">
            <template v-slot:default="{ row }">
              {{ $t('runtime.health_check.' + row.method) }}
            </template>
          </el-table-column>
          <el-table-column prop="result" :label="$t('runtime.health_check.result')">
            <template v-slot:default="{ row }">
              {{ $t('runtime.health_check.' + row.result) }}
            </template>
          </el-table-column>
          <el-table-column prop="msg" :label="$t('runtime.health_check.message')" />
        </el-table>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" v-if="checkData.level == 'STATUS_ERROR'" @click="onRecover()" v-preventReClick>
          {{ $t('runtime.health_check.recover') }}
        </el-button>
      </div>
    </el-dialog>
  </layout-content>
...
import TaskLogs from "@/components/task-logs/index.vue"
...
export default {
  name: "TaskList",
  components: { ComplexTable, LayoutContent, TaskLogs },
  data() {
    return {
      buttons: [
        {
  ...
  methods: {
    ...
    search(condition) {
      ...
      searchTasks(currentPage, pageSize, condition, false).then((data) => {
        this.loading = false
        this.data = this.setStatus(data.items || [])
        this.paginationConfig.total = data.total
      })
    },
    setStatus(items) {
      items.forEach(element => {
        element.status = element.status.status
      });
      return items
    },
    searchForPolling(condition) {
      const { currentPage, pageSize } = this.paginationConfig
      searchTasks(currentPage, pageSize, condition, true).then((data) => {
        this.data = this.setStatus(data.items || [])
        this.paginationConfig.total = data.total
      })
    },
    ...
    // onImport() {
    //   this.$router.push({ name: "TaskImport" })
    // },
    onUpgrade(row) {
      if (!row) {
        row = this.clusterSelection[0]
      }
      this.$router.push({ name: "TaskUpgrade", params: { name: row.name } })
    },
    goForDetail(row) {
      listRegistryAll().then((data) => {
        ...
        if (isExit) {
          this.$router.push({ name: "TaskOverview", params: { project: row.projectName, name: row.name } })
        } else {
          this.$message({ type: "info", message: this.$t('runtime.creation.repo_err') })
        }
      })
    },
    selectChange() {
      let isOk = true
      console.log('this.clusterSelection.length :>> ', this.clusterSelection);
    ...
    getStatus(row) {
      ...
      this.operationType = (row.status.indexOf("NotReady") !== -1) ? "not-ready" : "create-task"
      this.dialogLogVisible = true
      this.taskName = row.name
    },
    onRetry(preStatus) {
      console.log('onRetry preStatus:>> ', preStatus);
      retryTask(this.taskName)// TODO: 分类 retry
      // switch (preStatus) {
      // case "Upgrading":
      //   upgradeCluster(this.taskName, this.currentCluster.spec.upgradeVersion)
      //   break
      // case "Initializing":
      //   initCluster(this.taskName)
      //   break
      // case "Terminating":
      //   deleteCluster(this.taskName, true, this.isUninstall)
      //   break
      // case "Creating":
      // initCluster(this.taskName)
      // retryTask(this.taskName)
      // break
      // case "Waiting":
      //   initCluster(this.taskName)
      //   break
