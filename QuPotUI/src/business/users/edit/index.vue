<template>
  <layout-content :header="$t('commons.button.edit')" :back-to="{ name: 'UserList' }">
      ...
      <el-col :span="10">
        <div class="grid-content bg-purple-light">
          <el-form ref="form" :model="form" :rules="rules" label-width="80px" label-position="left">
            ...
            <!-- <el-form-item v-if="currentUser.isSuperuser && form.username !== 'admin'" :label="$t('user.role')"
              prop="role">
            ...
            <el-form-item v-if="currentUser.isSuperuser && form.username !== 'admin'">
    ...
    <el-dialog :title="$t('login.reset_password')" :close-on-click-modal="false" :visible.sync="dialogVisible"
      width="30%">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="onSubmitChange('passwordForm')" v-preventReClick>{{ $t("commons.button.submit")
        }}</el-button>
      </span>
    </el-dialog>
  </layout-content>
...
import { resetPassword } from "@/api/personal-setting"
...
export default {
  ...
  methods: {
    ...
    onChange() {
      ...
      this.passwordForm.id = this.form.username
    ...
    onSubmitChange(formName) {
      this.$refs[formName].validate((valid) => {
        ...
        resetPassword(this.form.username, {
  ...
  created() {
    ...
    getUser(this.username).then(data => {
