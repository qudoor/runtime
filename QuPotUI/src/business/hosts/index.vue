<template>
  <layout-content :header="$t('runtime_host.host')">
    <complex-table :data="data" ref="hostData" :row-key="getRowKeys" local-key="host_columns" @selection-change="selectChange" :pagination-config="paginationConfig" @search="search" :selects.sync="hostSelections" v-loading="loading" >
      <template #header>
        <el-button-group>
...
// import { importHosts } from "@/api/hosts"
import { deleteHost, searchHosts, openTerminal, syncHosts } from "@/api/hosts"
    ...
    return {
      buttons: [
        ...
        },
        {
          ...
          click: (host) => {
            console.log('row host: ', host)
            openTerminal(host.ip, host.port, host.credentialId)
          },
          disabled: (row) => {
