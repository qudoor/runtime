<template>
  <layout-content>
    <complex-table local-key="system_log_columns" :header="$t('system_log.title')" :data="data"
      :search-config="searchConfig" :pagination-config="paginationConfig" @input="search" @search="search">
      ...
      <el-table-column sortable :label="$t('system_log.operation_info')" min-width="100" prop="operationInfo"
  ...
  methods: {
    search(condition) {
      ...
      systemQuery(currentPage, pageSize, condition).then((response) => {
        ...
        console.log('qupotUI this.data :>> ', this.data);
