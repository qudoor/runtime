...
if [ ${OS} == "Darwin" ]; then
    ...
    yum -y install wget gcc-c++ unzip perl python-devel python3-devel perl-devel 'perl(Data::Dumper)' 'perl(Thread::Queue)' git patch
elif [ $ID == "centos" ]; then
    yum -y install wget gcc-c++ unzip perl python-devel python3-devel perl-devel git patch
elif [ $ID == "ubuntu" ]; then
    apt-get update
    apt-get install git wget gcc g++ make unzip  python-dev-is-python3 patch -y
fi
